...
describe('lib/util.js', function() {
	...
	describe('#sanitizeSymbolName', function() {
		...
		it ('should replace space', function(){ 
			util.sanitizeSymbolName('Foo 1').should.equal('Foo1');
		});
		it ('should replace ^', function(){ 
			util.sanitizeSymbolName('Foo^').should.equal('Foo');
		});
		it ('should replace *', function(){ 
			util.sanitizeSymbolName('Foo *').should.equal('Foo');
		});
		...
		it ('should replace .', function(){ 
			util.sanitizeSymbolName('Foo.Bar').should.equal('Foo_Bar');
		});
	});

	describe('#multilineComment', function(){
		var comment = util.multilineComment('hello');
	...
	describe('#filelisting', function(){
		var dir = 'test'+(+new Date);
		var dirs = ['.git','.svn','CVS','.DS_Store','foo','bar'];
		var files = ['foo/a','bar/b.txt'];
		dirs.forEach(function(name){
			wrench.mkdirSyncRecursive(path.join(dir,name));
		});
		...
		it('should be able to list all files', function(){
			var items = util.filelisting(dir);
			items.should.have.length(2);
			files.forEach(function(fn){
				items.should.containEql(path.resolve(path.join(dir,fn)));
			});
		});
		it('should be able to filter files with regular expression', function(){
			var items = util.filelisting(dir,/\.txt$/);
			items.should.have.length(1);
			items.should.containEql(path.resolve(path.join(dir,'bar/b.txt')));
		});
		it('should be able to filter files with function', function(){
			var filter = function(f,d) {
				return path.basename(f)==='a' && path.basename(d)=='foo';
			}
			var items = util.filelisting(dir,filter);
			items.should.have.length(1);
			items.should.containEql(path.resolve(path.join(dir,'foo/a')));
		});
	});
