...
function run(command, options, platform, args, done) {
	...
	if (options.debug && !orig_options['log-level']) {
	...
	state.hook.emit('pre.execute', state);

	cmd.executionStartedAt = Date.now();
	cmd.execute(state, function(err, results) {
		if (err) {
			state.hook.emit(command+'.failed', err, results);
		...
		state.hook.emit('post.execute', state, results);
		finishedCommand(cmd);
		if (done) {
			done(err, results);
		}
