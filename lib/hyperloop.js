...
function run(command, options, platform, args, done) {
	var found = _.contains(getCommands(platform), command);
	if (!found) {
	...
	[options.src, options.platform_dir, process.cwd()].forEach(function(d){
...
function getCommands(platform) {
	...
	try {
		var commandsPath = path.join(platform.directory,'commands');
...
function getCommand(platform, command) {
	var commandPath = path.join(platform.directory,'commands',command);
	var obj = require(commandPath);
	// var cmd = new Command(command, obj.description, obj.options||[], obj.execute);
	return obj;
}
