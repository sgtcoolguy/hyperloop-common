		...
		if (node instanceof Uglify.AST_New) {
			...
			if (node.left.hyperloop && node.left.hyperloop.type==='getter') {
				...
				if (right.hyperloop && right.hyperloop.type==='setter') {
						...
						if (cls && !jsgen.isBuiltinFunction(field) && library.findProperty(state.metabase, cls, field)) {
							...
							// We want to return two statements: the old assignment first, then our new assignment using the getter
							// but I can't seem to figure out the right AST structure for that
							//return new Uglify.AST_BlockStatement({
							//	body: [oldAssign, result]
							//});
							return new Uglify.AST_SimpleStatement({body:new Uglify.AST_Array({elements:[oldAssign, result]})});
						}
				}
				if (cls && !jsgen.isBuiltinFunction(field) && library.findProperty(state.metabase, cls, field)) {
			...
			if (node.left.thedef && node.left.thedef.orig) {
				...
				if (val && val.returnType) {
					var match = val.returnType.match(/^(void|float|int|long|short|double|bool|char)\s\*$/);
					if (match) {
