...
function isJSBuiltinObject(name) {
	return /^(JSON|Math|Number|String|Date|RegExp|(Float(32|64)|Int(16|32|8)|Uint(16|32|8|8Clamped))?Array|ArrayBuffer|Object|Function|Boolean|Error|require)$/.test(name) ||
			isMochaObject(name);
}
...
function splitClassWithStaticField(library, state, name) {
	...
	var property = _.clone(library.findProperty(state.metabase, t1, t2, true));
	if (property) {
...
function uglifyNodeToHyperloopNode (arg, node){ 
	...
	else if (arg instanceof Uglify.AST_Call){
		return {
			type: 'variable',
			value: arg.print_to_string()
		};
...
function compile (options, state, library, arch, source, filename, jsfilename, build_opts) {
	...
	var transformer = new Uglify.TreeTransformer(null, function(node, descend){
		...
		if (node instanceof Uglify.AST_New) {
		...
		else if (node instanceof Uglify.AST_This) {
			if (state.inProcessCustomClass) {
				var cls = state.inProcessCustomClass.defineClass[0].value;
				state.node_map[JSON.stringify(node.start)] = {class:cls, returnType:cls, variable:'this'};
			}
		}
		else if (node instanceof Uglify.AST_Dot) {
			if (node.print_to_string() == 'this.super') {
				if (state.inProcessCustomClass) {
					var cls = state.inProcessCustomClass.defineClass[0].value;
					var supercls;
					if (state.inProcessCustomClass.extends) {
						supercls = state.inProcessCustomClass.extends[0].value;
					} else if (state.inProcessCustomClass.implements) {
						supercls = state.inProcessCustomClass.implements[0].value;
					}
					state.node_map[JSON.stringify(node.start)] = {class:cls, superclass:supercls, returnType:cls, variable:'super'};
				}
			}
			...
			// search for constant. constant symbols are transformed into CONSTANTNAME_Get() function.
			if (library.findSymbol) {
				var typeInfo = library.findSymbol(state.metabase, node.name);
				if (typeInfo) {
					var type = typelib.resolveType(node.name);

					if (type.isNativeConstant()) {
						var getter = node.name + '_Get';
						state.builtin_symbols = state.builtin_symbols || {};
						state.builtin_symbols[getter] = node.name;
						return new Uglify.AST_Call({
							args: [],
							expression: new Uglify.AST_SymbolRef({name:getter,start:node.start,end:node.end}),
							start: node.start
			...
			if (nodeRef) {
				...
				// this.super method call
				if (nodeRef.variable === 'super') {
					return generateSuperMethodCall(library, state, nodeRef['superclass'], nodeRef['class'], node.expression.property, node);
				}
				...
				// If symbol is not found, this could be a developer error by calling a function that does not exit
				// or it could be a JS function attached to the object. Let the JS engine handle it
				if(!symbol) return;
				...
				if (node.expression && node.expression.thedef && node.expression.thedef.undeclared){
					if (node.expression && name && !isBuiltinObject(name) && library.isValidSymbol(state, name)) {
						return generateFunctionCall(library, state, name, node);
					} else if (!isJSBuiltinObject(name)) {
						fail(node, name.yellow+" is an undefined symbol");
					}
				}
...
function compress (ast, build_opts, filename, target, options, state) {
	...
	/*
	 * commeting for now - symbols not in the current code might be in some other js file
	 *
...
function generateSuperMethodCall(library, state, superclass, classname, methodname, node) {
	var fn = classname+'_super_'+methodname,
		key = state.obfuscate ? jsgen.obfuscate(fn) : fn;
	var varname = node.start.value,
		expr = node.expression.property,
		symbol = library.getInstanceMethodSymbol(state,superclass,expr,varname,fn,node,fail);
	...
	state.supermethods = state.supermethods || {};
	state.supermethods[classname] = state.supermethods[classname] || {};
	state.supermethods[classname][key] = symbol;

	var args = node.args;
	var func_name = key;
	args.unshift(new Uglify.AST_SymbolFunarg({name:'this', start:node.start}));
	return new Uglify.AST_Call({
		args: args,
		expression: new Uglify.AST_SymbolRef({name:func_name}),
		start: node.start
	});
}
...
function generateFunctionCall(library, state, name, node) {
	var fn = library.getFunctionCallName ? library.getFunctionCallName(state,name,node) : jsgen.generateFunctionCallName(name),
...
function compileCommand(options, state,library,arch,node,command,dict) {
	switch (command) {
		case 'defineClass': {
			if (dict.build) {
				state.inProcessCustomClass = undefined;
				...
				state.inProcessCustomClass = dict;
				return;
			}
		}
		case 'method': {
		...
		case 'registerSubclass': {
			// hack the dictionary to match defineClass style
			var newDict = {};
			newDict['defineClass'] = [{'type': 'variable', 'value': dict.registerSubclass[0].value.name.value}];
			if (dict.registerSubclass[0].value.protocols) {
				newDict['protocol'] = dict.registerSubclass[0].value.protocols.value;
			}
			if (dict.registerSubclass[0].value.superclass) {
				newDict['implements'] = [dict.registerSubclass[0].value.superclass];
			}
			// Transform methods
			var newMethods = _.map(dict.registerSubclass[0].value.methods.value, function(definition, name) {
					definition.value.name = {"type": "value", "value": name, "metatype": "string"};
					definition.value.action = definition.value.implementation;
					delete definition.value.implementation;
					if (definition.value.types) {
						definition.value.returns = definition.value.types.value[0];
						definition.value.arguments = {type: 'array' };
						definition.value.arguments.value = _.map(definition.value.types.value[1].value, function(typeName, idx) {
							return {type: "dict", "value": {"name": {"type": "value", "value": "arg"+idx, "metatype": "string"}, "type": {"type": "value", "value": typeName.value, "metatype": "string"}}};
						});
						delete definition.value.types;
					}
					return definition;
			});
			newDict['method'] = newMethods;
			newDict.build = {};
			// Now call back like we used 'defineClass'
			return compileCommand(options, state,library,arch,node,'defineClass',newDict);
		}
		default: {
