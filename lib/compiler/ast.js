...
function fail(node, msg, extra) {
	log.fatal(msg+" at "+(node.start.file+":"+node.start.line).green+(extra?"\n\n"+extra:''));
}
...
function isJSBuiltinObject(name) {
	return /^(JSON|Math|String|Date|RegExp|Array|Object|require)$/.test(name);
}
...
function splitClassWithStaticField(library, state, name) {
	...
	if (property) {
		if (property.instance === false || typeof(property.instance)==='undefined') {
			...
			log.fatal('Invalid field access for '+name+', this is not a static field');
		}
	}
	return null;
}
...
function hyperloopNodeToUglifyNode (arg) {
	switch (arg.type) {
		...
		case 'boolean': {
			return c.value ? new Uglify.AST_True() : new Uglify.AST_False();
		}
		case 'expression':
		case 'eval': 
		case 'value': {
			switch (arg.metatype || (arg.start && arg.start.type) || typeof(arg.value)) {
				...
				default: {
					log.fatal('not sure how to handle',arg)
				}
			}
		...
		default: {
			//TODO
			log.fatal("argument conversion to AST not supported for",arg);
		}
	}
}
...
function uglifyNodeToHyperloopNode (arg, node){ 
	if (arg.elements) {
		...
		switch (arg.start.value) {
			...
			default: {
				log.fatal("don't know how to deal with atom",arg);
			}
		}
	}
	...
	log.fatal("i don't know what this is",arg);
}
...
function getNodeContent(source, node) {
	return source.substr(node.start.pos, (node.end.endpos-node.start.pos));
}

function getInitClassFromSource(source, node) {
	if (node.thedef && node.thedef.init && node.thedef.init.expression) {
		var expr = node.thedef.init.expression;
		return source.substr(expr.start.pos, (expr.end.endpos-expr.start.pos));
	}
	return undefined;
}
...
function compile (options, state, library, arch, source, filename, jsfilename, build_opts) {
	...
	var dumpAST = build_opts['DUMP-AST'];
	...
	var transformer = new Uglify.TreeTransformer(null, function(node, descend){
		...
		if (node instanceof Uglify.AST_New) {
			...
			var varStatic = splitClassWithStaticField(library, state, getNodeContent(source, node));
			if (varStatic) {
				return generateStaticGetter(library, state, varStatic.name, varStatic.property, node);
			}
			var cls = getInitClassFromSource(source, node.expression);
			var field = node.property;
			var instance = node.expression;

			if (!cls && node.expression.hyperloop && node.expression.hyperloop.property) {
				var expr_type = typelib.resolveType(state.metabase, node.expression.hyperloop.property.type, options.platform);
			...
			if (node.left.hyperloop && node.left.hyperloop.type=='getter') {
				...
				if (cls && !jsgen.isBuiltinFunction(field) && library.findProperty(state.metabase, cls, field)) {
					return generateSetterCall(library, state, cls, field, node.left.hyperloop.instance, node.right, node);
				}
			}
		}
			...
			if (node.start.value === 'Hyperloop') {
				...
				if (command) {
					cmdnode = compileCommand(options,state,library,arch,node,command,dict);
					if (cmdnode) {
						return cmdnode;
					}
				}
			...
			if (node.expression && node.expression.property==='cast') {
				...
				state.node_map[JSON.stringify(node.start)] = {returnType:cast_to};
				var newnode = new Uglify.AST_SymbolRef({name:node.expression.start.value});
			...
			if (node.expression && name && !isBuiltinObject(name) && library.isValidSymbol(state, name)) {
				...
				if (undeclared && name && !isBuiltinObject(name) && library.isValidSymbol(state, name)) {
					...
					if (lookup) {
						name = (lookup.type === 'constructor'||lookup.type==='method') ? lookup['class'] : lookup['returnType'];
...
function generateNewConstructor(library, state, name, node) {
	var fn = jsgen.generateNewConstructorName(name),
		key = state.obfuscate ? jsgen.obfuscate(fn) : fn;
	//TODO: move into library
	state.symbols[key] = {type:'constructor',metatype:'constructor',symbolname:fn,class:name,location:node.start,argcount:node.args.length};
...
function generateMethodCall(library, state, name, node) {
	...
	state.symbols[key] = {type:'method',metatype:'instance',symbolname:fn,instance:varname,class:name,name:expr,location:node.start,argcount:node.body.args.length};
	state.node_map[JSON.stringify(node.start)] = state.symbols[key];
	node.body.args.unshift(new Uglify.AST_SymbolFunarg({name:varname}));
	node.body.expression = new Uglify.AST_SymbolRef({name:key});	
...
function generateMethodCall3(options, library, state, methodinfo, node){
	var varname = node.start.value,
		expr = node.expression.property,
		name = typelib.resolveType(state.metabase, methodinfo.method.method.returnType, options.platform).toName(),
		fn = jsgen.generateMethodName(name, expr),
		symbol = library.getInstanceMethodSymbol(state,name,expr,varname,fn,node,fail),
		key = state.obfuscate ? jsgen.obfuscate(fn) : fn;
	state.symbols[key] = symbol;
...
function compileCommand(options, state,library,arch,node,command,dict) {
	switch (command) {
		case 'defineClass': {
			if (dict.build) {
				var result = library.defineClass(options,state,arch,node,dict),
				...
				return new Uglify.AST_SimpleStatement({body:new Uglify.AST_Array({elements:elements})});
			} else {
				return;
			}
		}
		case 'method': {
			if (dict.call) {
				...
				var result = library.defineMethod(options,state,arch,node,dict);
				var args = result.args.map(function(c) {
					return hyperloopNodeToUglifyNode(c);
				});
				return new Uglify.AST_Call({args:args, expression:new Uglify.AST_SymbolRef({name:result.name})});
			} 
			else {
				return;
			}
