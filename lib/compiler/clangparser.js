...
const DEBUG_TREE = false,
		...
		VECTOR_SIZE = /__attribute__\(\(__vector_size__\((\d+) \* sizeof\((.*)?\)\)\)\)/,
		NEON_VECTOR_SIZE = /__attribute__\(\((neon_(poly)?vector_type)\((\d+)\)\)\)/,
		OPENCL_VECTOR_SIZE = /__attribute__\(\((ext_vector_type)\((\d+)\)\)\)/;
...
function parseTypes(type) {
	...
	type = formatType(dequote(tok.length===1 ? tok[0] : tok[1])) || subtype;
...
function splitAtFirstToken(line, token) {
	...
	var first = i > 0 ? line.substring(0,i).trim() : line,
...
function Node(index, line, start) {
	...
	if (this.line) {
		this.remainder = this.line.substring(this.start+1);
		var tok = this.remainder.split(' ');
...
Node.prototype.parseTypedefDecl = function() {
	...
	this.type = formatType(tok.subtype);
	this.subtype = formatType(tok.type);
	this.metatype = 'typedef';

	if (VECTOR_SIZE.test(this.line)) {
		this.vector = true;
		var m = VECTOR_SIZE.exec(this.line);
		this.vector_size = parseInt(m[1]);
		this.vector_type = m[2];
	}
	else if (NEON_VECTOR_SIZE.test(this.line)) {
		this.vector = true;
		var m = NEON_VECTOR_SIZE.exec(this.line);
		this.vector_type = m[1];
		this.vector_size = parseInt(m[3]);
	}
	else if (OPENCL_VECTOR_SIZE.test(this.line)) {
		this.vector = true;
		var m = OPENCL_VECTOR_SIZE.exec(this.line);
		this.vector_type = m[1];
		this.vector_size = parseInt(m[2]);
	}
	...
	if (!this.framework) {
		...
		if (!this.framework && (i = this.header && this.header.indexOf('.sdk/')) > 0) {
			var imp = this.header.substring(i+4),
				p = path.basename(imp);
			// skip private headers that begin with _
			if (p.charAt(0)!='_') {
				this.import = imp;
			}
		}
		if (!this.import && (i = this.header && this.header.lastIndexOf('/include/')) > 0) {
			var imp = this.header.substring(i+9),
				p = path.basename(imp);
			// skip private headers that begin with _
			if (p.charAt(0)!='_') {
				this.import = imp;
		...
		if (this.import && /^\/usr\/include\//.test(this.import)) {
			this.import = this.import.substring('/usr/include/'.length);
		}
	}
...
Node.prototype.parseCXXRecordDecl = function() {
	this.name = dequote(this.field).split(' ')[1].split(' ')[0];
	if (this.field.indexOf('definition') === -1) {
...
Node.prototype.parseCXXConstructorDecl = function() {
	if (this.field[0] === "'") {
		return;
	}
	this.instance = false;
	var tok = splitAtFirstToken(this.field),
		name = tok.first,
		tok2 = splitAtFirstToken(tok.next),
		retVal = tok2.first;
	this.selector = '+' + this.field;
	this.returnSubtype = formatType(name);
	this.returnType = formatType(name);
	this.name = '.ctor';
};
Node.prototype.parseCXXConstructorDeclAfterChildren = function() {
	if (this.field[0] === "'") {
		return;
	}
	Node.prototype.parseObjCMethodDeclAfterChildren.call(this);
	var methods = this.parent.methods[this.name],
		method = methods[methods.length - 1];
	// Mark it as public or private.
	// TODO: How do we actually tell if it's public or not?
	// TODO: Properly parse the args.
	method.attributes = [ method.args.length && this.name === '.ctor' ? 'private' : 'public' ];
};
Node.prototype.parseCXXMethodDecl = function() {
	if (this.field[0] === "'") {
		return;
	}
	this.instance = false;
	var tok = splitAtFirstToken(this.field),
		name = tok.first,
		tok2 = splitAtFirstToken(tok.next),
		tok3 = splitAtFirstToken(tok2.first);
	this.selector = '-' + this.field;
	this.returnSubtype = formatType(tok3.next);
	this.returnType = formatType(tok3.first);
	this.name = name;
};
Node.prototype.parseCXXMethodDeclAfterChildren = function() {
	Node.prototype.parseCXXConstructorDeclAfterChildren.call(this);
};
Node.prototype.parseObjCMethodDecl = function() {
	...
	this.returnSubtype = formatType(dequote(tok.first));
	this.returnType = formatType(dequote(tok.next||tok.first));
	this.name = name;
};
...
Node.prototype.parseObjCMethodDeclAfterChildren = function() {
	...
	if (this.children) {
		this.children.forEach(function(child) {
			...
			else if (child.token === 'ParamVarArg') {
				hasVarArgs = true;
			}
	...
	if (methodObj.returnSubtype==='instancetype') {
		// instancetype means return instance of this class
		var classParent = this.up(function(n){
			return n.token === 'ObjCInterface' && n;
		});
		classParent && (methodObj.returnType = classParent.name);
	}
...
Node.prototype.parseFunctionDecl = function() {
	...
	this.framework = parseFrameworkFromHeader(this.header);
	if ((this.header && this.header.indexOf('.h')===-1) && !this.framework) {
...
Node.prototype.parseFunctionDeclAfterChildren = function() {
	...
	if (this.children && this.children.length) {
		this.children.forEach(function(arg){
			if (arg.token === 'ParmVarDecl') {
				...
				if (arg.type==='') arg.type = arg.name;
				if (arg.subtype==='') arg.subtype = arg.name;
				var e = {
					name: arg.name,
					type: arg.type,
					subtype: arg.subtype
	...
	if (!this.framework) {
		var i = -1;
		// if this looks like a non-framework include, we should add a specific import
		if (!this.framework && (i = this.header && this.header.indexOf('/usr/include/')) >0) {
...
Node.prototype.parseIntegerLiteral = function() {
	...
	if (primitiveRegex.test(this.type) && value !== 'invalid'){
...
Node.prototype.parseBinaryOperatorAfterChildren = function() {
	...
	try {
		...
		log.debug('error evaluating expression; ignoring: ', expr);
		//log.debug(E.stack);
		this.value = 0;
	}
};
...
Node.prototype.parse = function() {
	...
	if (this.token === '...') {
		...
		var remainder = this.remainder.substring(this.token.length+this.addr.length+2);
...
const KEY_BLACKLIST = ['parent', 'sibling', 'addr', 'line', 'field', 'index', 'start', 'children', 'begin', 'end', 'header', 'token', 'extra', 'length', 'remainder'];
...
function astToJSON (ast) {
	...
	json.thirdparty_frameworks = thirdpartyFrameworks;
	json.system_frameworks = systemFrameworks.map(function(f){return f.name});
	json.system_frameworks_map = {};

	systemFrameworks.forEach(function(f){
		json.system_frameworks_map[f.name] = f.relative;
	});
	for(var f in exports.otherFrameworks) {
		if(!json.system_frameworks_map[f]) {
			json.system_frameworks_map[f] = exports.otherFrameworks[f].relative;
		}
	}
...
function parseFile(buildlib, fn, callback) {
	var byline = require('byline'),
		output = fs.createReadStream(fn),
		stream = byline(output);
	parse(buildlib, stream, output, callback);
}

function parseBuffer(buildlib, buf, callback) {
	var byline = require('byline'),
...
function parseSystemFrameworks(buildlib, callback) {
	if (systemHeaders) return callback(null,systemHeaders,systemframeworkDir,systemFrameworks);

	processSystemFrameworks(buildlib, function(err,result,frameworks,frameworkDir){
...
function parse(buildlib, stream, pipe, callback) {

	parseSystemFrameworks(buildlib, function(err,systemHeaders,frameworkDir,systemFrameworks){
		if (err) return callback(err);
		...
		stream.on('data',function(line){
			...
			while (m) {
				var f = m[1],
					fn = f+'.framework';
				if (systemFrameworks.indexOf(fn)===-1 && !(f in thirdpartyFrameworks)) {
					var i = line.lastIndexOf('<',m.index),
						n = i != -1 && line.indexOf(':',m.index),
						p = i != -1 && line.substring(i+1,n),
						t = i != -1 && p.split(',').filter(function(e){ return e.indexOf(fn)!==-1 }),
						p = t && t.length && t[0].trim();
					if (p && p.indexOf(frameworkDir)===-1) {
						thirdpartyFrameworks[f] = path.dirname(p);
					}
				}
			...
			if (start > 0) {
				if (current.start === start) {
					DEBUG_TREE && log.debug(util.rpad(25, util.rpad(4,index-1), '|', util.rpad(4,start), '|', util.rpad(10,'sibling'), '|', util.rpad(3,current.index)), '|', util.rpad(20,node.token), line.substring(0,80));
					current.parent.add(node);
				}
				else {
					if (current.start < start) {
						DEBUG_TREE && log.debug(util.rpad(25, util.rpad(4,index-1), '|', util.rpad(4,start), '|', util.rpad(10,'child'), '|', util.rpad(3,current.index)), '|', util.rpad(20,node.token), line.substring(0,80));
						current.add(node);
					}
					else {
						...
						while(p) {
							if (p.start === start) {
								DEBUG_TREE && log.debug(util.rpad(25, util.rpad(4,index-1), '|', util.rpad(4,start), '|', util.rpad(10,'sibling'), '|', util.rpad(3,p.index)), '|', util.rpad(20,node.token), line.substring(0,80));
								p.parent.add(node);
								break;
							}
							p = p.parent;
				...
				DEBUG_TREE && log.debug(util.rpad(25, util.rpad(4,index-1), '|', util.rpad(4,start), '|', util.rpad(10,'root'), '|', util.rpad(3,-1)), '|', util.rpad(20,node.token), line.substring(0,80));
				root = node;
			}
			current = node;
			(index%10000==0) && log.trace('clang parsed',index);
		});

		function finish() {
			log.debug('clang parser processed contents');

			// parse any third-party frameworks that we found during the parse
			Object.keys(thirdpartyFrameworks).forEach(function(fn){
				var headersDir = thirdpartyFrameworks[fn],
					headerfiles = fs.readdirSync(headersDir);
				processIncludeDir(headersDir, headerfiles, systemHeaders, fn);
			});

			if (root) {
				log.trace('started clang parsing');
				root.parse();
				log.trace('finished clang parsing');
				callback(null, root);
...
function processIncludeDir(headersDir, headerList, headers, framework) {
	...
	headerList.forEach(function(sh){
		...
		if (headerFileRE.test(sh)) {
			...
			var hc = fs.readFileSync(hf,'utf8').toString(),
 ...
 */
function processSystemFrameworks(buildlib, callback) {

	buildlib.systemFrameworks(function(err,frameworks,frameworkDir){
		if (err) return callback(err);
		...
		frameworks.forEach(function(entry){
			var headerfiles = fs.readdirSync(entry.directory);
			if (headerfiles.length > 0){
				processIncludeDir(entry.directory, headerfiles, headers, entry.name);
			}
			else {
				log.error("Couldn't find any files in "+entry.directory);
			}
		});
