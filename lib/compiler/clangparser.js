...
Node.prototype.parse = function() {
	...
	if (this.token === '...') {
		...
		} else if (this.field.match(/^col:/) || this.field.match(/^line:/)) {
			this.field = this.field.substring(this.field.indexOf(' ')+1);
		}
...
function parse(buildlib, stream, pipe, callback) {
	...
	parseSystemFrameworks(buildlib, function(err,systemHeaders,frameworkDir,systemFrameworks){
		...
		stream.on('data',function(line){
			...
			index++;
			line = String(line); //  Don't trim this line otherwise 'start' variable below goes wrong
			//console.error(line);
