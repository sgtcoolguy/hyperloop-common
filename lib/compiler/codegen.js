...
function generateLibrary (options, build_opts, arch, state, library, uncompiledFiles, compiledFiles, callback) {
	...
	Object.keys(uncompiledFiles).forEach(function(key){
		var relative = makeRelative(path.relative(options.src,key)),
			obj = uncompiledFiles[key],
			root = path.resolve(options.src) == path.resolve(path.dirname(key)),
			dirname = makeRelative(path.dirname(relative));

		filemap.push({
			compiled: false,
			source: uncompiledFiles[key].source,
			root: root,
			dirname: dirname,
			filename: relative,
			json: obj.json
