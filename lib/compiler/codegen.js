...
function generateRequire(indent, ir, id, filename, dirname, symbols, symbolnames, cleanup, jscode, code, jscodevar, moduleid) {
...
function generateSourceEmbed (library, options, state, platform_lib, filemap, externs) {
	...
	/**
	 * this code below is callback functions that Hyperloop will call to 
	 * manage void * -> JSValueRef mappings.  Since equality in JS is the value
	 * of the actual JSValueRef being the same (for native objects), we need to 
	 * re-use the JSObjectRef created for a specific pointer value.  These functions
	 * are used to manage the mapping.  In the initializer of the object definition,
	 * the pointer and JSObjectRef is set and in the finalizer it is removed.  
	 * when the <type>_JSValueRef converter routine is called, it will first look to 
	 * see if the pointer is in the map and if so, it will reuse it, otherwise, it
	 * will create it with JSObjectMake.
	 */

	ecode.push('static std::map<void*,JSValueRef> pointerValues;');
	ecode.push('');
	ecode.push('static JSValueRef PointerToJSValueRef(void *pointer)');
	ecode.push('{');
	ecode.push('\tstd::map<void*,JSValueRef>::const_iterator v = pointerValues.find(pointer);');
	ecode.push('\tif (v!=pointerValues.end())');
	ecode.push('\t{');
	ecode.push('\t\treturn v->second;');
	ecode.push('\t}');
	ecode.push('\treturn nullptr;');
	ecode.push('}');
	ecode.push('');
	ecode.push('static void RemoveJSValueRef(void *pointer)');
	ecode.push('{');
	ecode.push('\tstd::map<void*,JSValueRef>::const_iterator v = pointerValues.find(pointer);');
	ecode.push('\tif (v!=pointerValues.end())');
	ecode.push('\t{');
	ecode.push('\t\tpointerValues.erase(v);');
	ecode.push('\t}');
	ecode.push('}');
	ecode.push('');
	ecode.push('static void PointerSetJSValueRef(void *pointer, JSValueRef value)');
	ecode.push('{');
	ecode.push('\tRemoveJSValueRef(pointer);');
	ecode.push('\tpointerValues[pointer]=value;');
	ecode.push('}');
	ecode.push('');
	...
	filemap.forEach(function(fe){
		...
		if (fe.json) {
			...
			generateRequire('\t\t',fe.ir,id,fe.filename,fe.dirname,fe.symbols,fe.symbolnames,fe.cleanup,fe.source,ecode,varname,options.moduleid,options.debugsource);
			if (!fe.ir) {
				var define = jsgen.generateDefine(varname,jsgen.transform(fe.source,null,null,debugfn));
				defines.push('// '+fn+'\n'+define);
				genjs = true;
	...
	code.push('{');
	code.push('\tHyperloopRegisterTranslationUnit(&PointerToJSValueRef, &PointerSetJSValueRef, &RemoveJSValueRef, &HyperloopLoadEmbedSource,'+varnames.length+','+varnames.map(function(v){return '"'+v+'"';}).join(',')+');');
	code.push('}');
	code.push('');
...
function generateHeader (options, state, obj) {
	...
	code.push('#include <hyperloop.h>');
	code.push('');
	code.push('JSValueRef HyperloopLoad_'+obj.jsfilename+' (JSValueRef *exception);');
...
function generateMain (options, state, obj, symbols, symbolnames, externs, cleanup) {
	...
	code.push('EXPORTAPI JSValueRef HyperloopLoad_'+obj.jsfilename+' (JSValueRef *exception)');
	code.push('{');
	code.push('\treturn HyperloopLoadEmbedSource(InitializeHyperloop(),"'+obj.jsfilename+'",exception);');
	code.push('}');
