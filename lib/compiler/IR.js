...
IR.prototype.parse = function(state,arch,filename,jsfilename,relativeFilename,source,ast) {
	...
	// --skip_ir option basically does nothing special but returns whole code
	if (state.options.skip_ir) {
		var expr = self.createJSExpression(ast.print_to_string());
		expr.line = ast.start.line;
		annotateExpr(self,expr,ast);
		...
		if (node instanceof Uglify.AST_EmptyStatement) {
			...
			node.body.walk(new Uglify.TreeWalker(function(n){
				...
				if (n instanceof Uglify.AST_SimpleStatement || n instanceof Uglify.AST_Call) {
					...
					throw new Error("NOT HANDLED BODY "+n.TYPE+' => '+n.print_to_string().green)
				}
			}));
		}
