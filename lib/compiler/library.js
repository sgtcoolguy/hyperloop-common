	...
	if (symboltable.classmap) {
		...
		Object.keys(symboltable.classmap).forEach(function(name){
			...
			if (!clscode.length && isClassLike) {
				...
				clscode.push('{');
				clscode.push('\tauto n = ToNativeObject(object);');
				clscode.push('\tHyperloopPointerSetJSValueRef('+typeobj.toVoidCast('n')+',object);');
				...
				clscode.push('{');
				clscode.push('\tauto n = ToNativeObject(object);');
 			   	clscode.push('\tHyperloopRemovePointerJSValueRef('+typeobj.toVoidCast('n')+');');
				...
				clscode.push('\tauto v = HyperloopPointerToJSValueRef('+typeobj.toVoidCast('instance')+');');
				clscode.push('\tif (v!=nullptr) return v;'); // check to see if we already have an instance for this pointer and re-use if we do
