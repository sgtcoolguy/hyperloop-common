	...
	if (symboltable.classmap) {
		...
		Object.keys(symboltable.classmap).forEach(function(name){
			...
			if (!clscode.length) {
				...
				clscode.push('\tHyperloopPointerSetJSValueRef(static_cast<void *>(n),object);');
				...
				clscode.push('{');
				clscode.push('\tauto n = ToNativeObject(object);');
 			   	clscode.push('\tHyperloopRemovePointerJSValueRef(static_cast<void *>(n));');
				...
				clscode.push('{');
				clscode.push('\tauto v = HyperloopPointerToJSValueRef(static_cast<void *>(instance));');
