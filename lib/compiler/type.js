...
exports.__defineGetter__('classes',function(){
	return Object.keys(typeCache).map(function(k){
		var obj = typeCache[k];
		if (obj.isNativeObject() || obj.isNativeStruct()) {
			return obj.toName();
		}
	}).filter(function(v){
		return v;
...
Type.prototype.toCast = function(leaveCast) {
	...
	var result = leaveCast ? cast : cast.replace(/^const /,'').trim();
	return result;
}
...
Type.prototype.toAllocStruct = function(varname, fnname, selector, code) {
	...
	code.push(this.toCast()+' '+varname+' = ('+this.toCast()+')malloc(sizeof('+this.toClassName()+'));');
	code.push('memcpy('+varname+',&'+varname+'$,'+'sizeof('+this.toClassName()+'));');
};
