...
var isPrimitive = /^(const)?\s*?(un)?(signed)?\s*(float|int|short|double|long|long)\s*(long|double)?\s*([*]{0,2})$/,
...
Type.prototype._parse = function(metabase) {
	...
	if (isPrimitive.test(type) || (typeobj && isPrimitive.test(typeobj.type))) {
		...
		this._value = this._value || m[4] + (m[5] ? (' '+m[5]) : '');
		this._pointer = m[6] || defPointer;
		...
		if (this._fields.length) {
			this._fields.forEach(function(field){
				if (!field.rawtype) {
					...
					field.type = resolveType(metabase,field.type,self.platform);
				}
			});
		}
		if ((!this._fields || !this._fields.length) && !this.isPointer()) {
			this._value +=' *';
			this._pointer = '*';
		}
	}
		...
		this._functionReturnType = resolveType(metabase, m[1].trim(), this._platform);
		var self = this;
		var fnargs = m[2] && m[2].split(',') || [];
		this._functionArgTypes = fnargs.length ? fnargs.map(function(a){
			return resolveType(metabase, a.trim(), self.platform);
		}) : fnargs;
		return;
	}
}
 ...
 * library subclass
 */
function resolveType(metabase, type, platform) {
	if (typeof(metabase)==='string') {
		type = metabase;
		metabase = exports.metabase;
	...
	metabase = metabase || exports.metabase;
	// platform = platform || exports.platform; //FIXME
	...
	if (platform) {
		...
		var Subclass = require('../../../platforms/'+platform+'/lib/type').Class;
		typeobj = new Subclass(metabase,type,platform);
	}
	else {
		typeobj = new Type(metabase,type,platform);
	}
