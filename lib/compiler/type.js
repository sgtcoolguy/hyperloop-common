...
Type.NATIVE_CONSTANT = NATIVE_CONSTANT;
...
Type.prototype._parseTypedef = function(metabase, type) {
	...
	if (metabase.types && ((type in metabase.types) || (metabase.symbols && type in metabase.symbols)) ) {
		var typeobj = metabase.types[type] || metabase.symbols[type];
...
Type.prototype.isNativeConstant = function() {
	return this._metatype == NATIVE_CONSTANT;
}
...
Type.prototype.toJSExtern = function() {
	return 'EXPORTAPI JSValueRef '+this.toJSValueName()+'(JSContextRef,'+this.toCast()+',JSValueRef *);';
}
