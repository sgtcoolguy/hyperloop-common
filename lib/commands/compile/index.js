									...
									platform_compiler.initialize(options, build_opts, arch, details, settings, function(err,state) {
										...
										Object.keys(compiledSources).forEach(function(filename){
											...
											compileTasks.push(function(next){
												...
												hook.emit('pre.compile.source', event, function() {
													...
													tasks.push(function(_next){
														ast = compiler.compile(event.options, state, platform_compiler, arch, event.source, fn, jsfilename, event.build_opts);
														event.ast = ast;
													...
													// compress the code
													tasks.push(function(_next){
														sourceobj = compiler.compress(event.ast, event.build_opts, fn, target, event.options, state);
													...
													if (options.dump_ir) {
														tasks.push(function(_next){
															var fn = path.join(event.options.jsSrcDir,relativeFilename+'.ir');
													...
													// generate code
													tasks.push(function(_next){
														library.generateCodeDependencies(event.options,state,symboltable,relativeFilename,arch,event.symbols,nodefail);
													...
													tasks.push(function(_next){
														...
														fs.writeFile(srcFile, event.sourceobj.ast.print_to_string({beautify:true}),'utf8',_next);
													});
										...
										async.series(compileTasks, function(err) {
											...
											finishTasks.push(function(next){
												...
												library.generateCode(event.options,state,event.symboltable,arch,nodefail);
												hook.emit('post.generate.code', event, next);
											});

											finishTasks.push(function(next){
												// call finish which should now do the actual compiling, etc.
												platform_compiler.finish(event.options, event.build_opts, arch, state, event.uncompiledSources, event.compiledFiles, next);
											});
											...
											finishTasks.push(function(next){
												...
												codegen.generateLibrary(event.options, event.build_opts, arch, state, platform_compiler, event.uncompiledSources, event.compiledFiles, next);
											});
